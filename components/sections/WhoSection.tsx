      ...
      <div className="grid gap-10 md:grid-cols-2">
        <div>
          <h2 className="font-heading text-2xl font-semibold tracking-tight text-motus-textLight md:text-3xl">
            <span className="text-holo">{whoCopy.title}</span>
          </h2>
        ...
        <div className="grid gap-6 text-sm md:text-base">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-motus-textLight/80">
              Es para ti si…
            </p>
            <ul className="space-y-1.5 text-motus-textLight/90">
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-motus-textLight/60">
              {whoCopy.notFor.map((item) => (
                <li key={item} className="flex gap-2">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-motus-muted/50" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
